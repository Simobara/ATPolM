...
const ProvModalMod = ({ propShow, propClose, propRowID, propListaProvCodAdded = [], propRowProvincia }) => {
  ...
  const handleInputDigitChange = (e) => {
    ...
    if (cleanedInput.length === inputText.length) {
      // Verifica se non ci sono numeri o simboli
      setErrorDigit("");
    } else {
    ...
    <>
      <Modal show={propShow} size="lg" aria-labelledby="contained-modal-title-vcenter" top="true"  centered>
          ...
          <Row style={{ width: "100%" }}>
            <Col sm={12} md={12} lg={12}>
              <Row className="mb-4">
                ...
                <Col xs={12} md={4}>
                  <form>
                    <input type="text" placeholder="Cerca Provincia..." onChange={handleInputDigitChange} autoFocus className={inputBorderClass} value={inputValue} />
                  </form>
                  {errorDigit && <p style={{ color: "red", fontSize: "18px" }}>{errorDigit}</p>}
                  {provinceFound && inputBorderClass === "form-control is-valid" && <p style={{ color: "green", fontSize: "18px" }}>TROVATA PROVINCIA {`-->`} </p>}
                </Col>
                <Col xs={12} md={6} className="regModal">
                  <CodiceFormMod
                    propFrmData={(e) => {
        ...
        <Modal.Footer className="d-flex justify-content-center mt-4">
          <Button onClick={handleModProvincia} className="justify-content-around">
