...
const Table = ({ handleAddNewRecPopup, propRowData = [], propColumnData = [] }) => {
    ...
    return (
      <div>
        ...
        <span>
          <img
            src={arrowUp}
            onClick={() => sortData(column?.Header, "assending")}
            className={`arrow-down-btn ${isAscendingActive ? 'active-arrow' : ''}`}
            alt="imagePic"
          />
        </span>
        <span>
          <img
            src={arrowDown}
            onClick={() => sortData(column?.Header, "dessending")}
            className={`arrow-down-btn ${isDescendingActive ? 'active-arrow' : ''}`}
            alt="imagePic"
          />
        </span>
      </div>
  ...
  const renderCellData = (cell) => {
    ...
    const arrowContainerClass =
      selectedCell?.id === id && isArrowSelected
        ...
        <tr className="detail-wrapper">
          <td className="detail-sections">
            <div className="info-section info-section_text section-padding">
              <span className="color-lightcoral info-section_text">
                Id Annuncio : {id}
              </span>
              <span className="color-lightcoral info-section_text">
                Data Chiusura: {dataDiScadenza}
              </span>
            </div>
            <div className="info-section section-padding">
              <span className="color-lightcoral info-section_text_color">
                Plastica Riciclata
              </span>
            </div>
            <div className="header_value ">
              ...
              <span
                className="header_accessor section-padding"
                dangerouslySetInnerHTML={{ __html: descrizioneDetail }}
              ></span>
            </div>
            <div className="info-section padding-img">
              <div className="header_value">
                <span className="header section-padding">
                  Quantita disponibile :
                </span>
                <span className="header_accessor section-padding">
                  {quantita}
                </span>
              </div>
            ...
            <div className="header_value header_offerente">
              ...
              <div className="section-padding">
                <button onClick={() => handleOpenContactModal()} className="btn btn-primary mt-2">Contatta</button>
                {isContactModalOpen && isUtenteRegistered && (
                  <ModalContact propShow={isContactModalOpen} propClose={handleCloseContactModal} />
                )}
                {isContactModalOpen && !isUtenteRegistered && (
                  <ModalRegister propShow={isContactModalOpen} propClose={handleCloseContactModal} />
                )}
              </div>
            </div>
          </td>
  ...
  const handleFilterRowData = (e) => {
    if (e.idProvincia) {
      let getFilteredAnnunci = [...propRowData].filter(
        (p) =>
          p.localita.provinciaCodice.toUpperCase() ===
          e.idProvincia.toUpperCase()
      );
  ...
  const handleFilterRegioniData = (e) => {
    if (e.length > 0) {
      ...
      let arrayData = [];
      e.forEach((data) => { // sostituito map con forEach
        let result = getFilteredAnnunci.filter(
          (p) => p.localita.provinciaCodice.toUpperCase() === data
        );
        if (result.length > 0) {
  ...
  const handleFilterDataCitta = (e) => {
    if (e.idCitta) {
      let getfilteredCittaData = [...propRowData].filter(
        (p) => p.localita.id === e.idCitta
      );
              ...
              >
                {filterData(headerGroup.headers)?.map((column, columnIndex) => (
                  <th
                    ...
                    className={`table-header-item header-color-bg-font`}
                    {...column.getHeaderProps()}
                  >
                    {column?.Header === "Immagine" ||
                      column?.Header === "" ||
                      column?.Header === "Classe Waste"
                      ? column.render("Header")
                      : renderIdAnnuncio(column)}
                  </th>
                ))}
              </tr>
            ))}
