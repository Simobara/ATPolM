...
const AziendaService = () => {
    ...
    const addAzienda = async (username, email, password, ragioneSociale, indirizzo, codiceFiscale, pIva, telefono,
        fax, pec, idRuolo) => {
        try {
            const response = await axios.post(`${API_AUTH_URL}aggiungi-azienda-singola/` +
                `${idRuolo}`, {
                username,
                email,
                password,
